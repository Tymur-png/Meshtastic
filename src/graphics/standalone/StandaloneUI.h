...
class StandaloneUI
{
  ...
  private:
    ...
    int countRemoteNodes() const;
    static bool isRelayRole(meshtastic_Config_DeviceConfig_Role role);
    static const char *roleLabel(meshtastic_Config_DeviceConfig_Role role);
    // Collect remote-node table indices: relay-class first, then everyone else.
    void collectRemoteNodes(std::vector<size_t> &out, bool relaysFirst) const;
