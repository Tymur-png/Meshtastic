...
void StandaloneUI::drawNetwork(OLEDDisplay *display)
{
    ...
    if (countRouters() > 0) {
        display->setTextAlignment(TEXT_ALIGN_CENTER);
        display->drawString(display->width() / 2, display->height() - FONT_HEIGHT_SMALL * 2 - 6, "[OK - ретрансляторы]");
    }
    drawStatusBar(display);
}
...
{
    drawFrameBox(display, "РЕТРАНСЛЯТОР");
    display->setFont(FONT_SMALL);
    display->setTextAlignment(TEXT_ALIGN_LEFT);
    int16_t y = FONT_HEIGHT_SMALL + 10;
    const int16_t lineH = FONT_HEIGHT_SMALL + 4;
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (now > 0 && node->last_heard > 0) {
            snprintf(buf, sizeof(buf), "Пакет: %u сек назад", now - node->last_heard);
            display->drawString(6, y, buf);
        }
        return;
    }
    display->setTextAlignment(TEXT_ALIGN_CENTER);
    display->drawString(display->width() / 2, display->height() / 2, "Не найден");
}
